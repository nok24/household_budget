...
cp .dev.vars.example .dev.vars  # 初回のみ。Worker secrets (GOOGLE_CLIENT_SECRET / DRIVE_TOKEN_AES_KEY 等)
...
- D1 schema 変更（`functions/db/schema.ts`）を含む PR は:
  - `npm run db:generate` で migration SQL を生成（drizzle-kit が `__new_table` 経由で書く場合は手で `DROP/CREATE` に圧縮するなど目視確認）
  - PR マージ後に `npm run db:migrate:remote` を手動実行
  - 後方互換性のないスキーマ変更は `BREAKING CHANGE` フッターを付け、本番反映前にデータ移行手段（admin endpoint や手動 SQL）を用意してから merge する。
...
- 自動テストは未導入。
- 必要になったタイミングで **Vitest + Miniflare**（API）+ **Vitest + React Testing Library**（フロント）を入れる方針。
- 最初に書くべき候補:
  - `functions/lib/csv/mfTransactions.ts` の Shift_JIS / amount パース
  - `functions/lib/budgetConfig.ts` `functions/lib/overridesConfig.ts` の validate / migrate ロジック
  - `src/lib/aggregate.ts` の pure 集計関数
  - `src/lib/accountBalance.ts:computeMonthlyBalancesFromArray` の残高推定
