...
家族全員が個別に Drive を OAuth する案は、過去の問題と同じ罠 (グラント単位の権限分離) に戻る。admin (夫) 1人がサーバ側で OAuth → refresh token を D1 に AES-GCM 暗号化保存 → Functions が `https://www.googleapis.com/auth/drive.readonly` のみで代理アクセス。

クライアント (フロント) は Drive scope を一切要求しない (`openid email profile` のみ)。書き込み (`drive.file`) も使わなくなったので、過去にあった「同名ファイル増殖」「クライアント間 403」の問題そのものが消える。
...
- `sync_log` に `error` で記録 (フロントは `useSyncStatus` で見ている)
- 設定画面の AdminPanel で「Drive 接続: 未接続」表示 + 再接続ボタン (admin のみ)
- 非 admin には UI 露出させない (家族誰でも「Drive がエラーです」と通知される必要はない)
...
1. admin が AdminPanel (設定画面) から「同期」ボタン押下 → `POST /api/sync/transactions` (`requireAuth` のみで非 admin も呼べるが UI は admin にだけ出す)
2. Functions: `sync_log` に `status='running'` で INSERT (重複実行ロック、10 分超 stale なら新規許可)
3. Drive 上の家計簿フォルダの CSV 一覧を取得 (refresh_token から access_token 発行)
4. `csv_files.modified_time` と比較して差分のみダウンロード + パース
5. ファイル単位で `DELETE WHERE source_file_id = ?` → `INSERT` で transactions を再構築
   - `overrides` は `(source_file_id, mf_row_id)` 複合 PK なので CSV 再取込でも再リンクされる (sync.ts の `deleteFileTransactions` で `includeOverrides=false`)
6. orphan ファイル (Drive 側から消えた) は `transactions` + `overrides` を連動削除 (`includeOverrides=true`)
...
フロント (`src/lib/accountBalance.ts:computeMonthlyBalancesFromArray`) で純粋関数として計算。`useAppliedTransactions` 由来の override 適用済み配列を入力にして、アンカー月端から各月末の推定残高を遡って作る。

サーバ側計算にしない理由: 集計対象データはどのみち取引一覧 (`/api/transactions/all`) でフロントに全件届くので、アンカー残高だけサーバ側 endpoint を立てる利得が無い。pure 関数なのでテストが書きやすく、月切替のたびに再計算しても十分速い。
...
- **Worker のロジック (`functions/lib/csv/`, `functions/lib/budgetConfig.ts`, `functions/lib/overridesConfig.ts`)**: 純粋関数として Vitest unit テストを入れる予定
- **Worker の API**: Vitest + Miniflare で D1 をローカル実行してエンドポイント単位 (transactions / overrides / budget / assets / sync / admin/migrate)
- **フロント pure 集計 (`src/lib/aggregate.ts`, `src/lib/accountBalance.ts`)**: pure 関数なので Vitest で fixture ベースのスナップショット
- **移行エンドポイント (`/api/admin/migrate/budget` `/api/admin/migrate/overrides`)**: dry-run と本番取込の両方を確認するシナリオ
- **フロント UI**: 重要な箇所 (Categories / Budget / Transactions の集計値) のみスモークテストを必要に応じて
